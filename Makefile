...
LEDGER_ENABLED ?= true
BINDIR ?= $(GOPATH)/bin
highbury_BINARY = black
highbury_DIR = black
BUILDDIR ?= $(CURDIR)/build
SIMAPP = ./app
HTTPS_GIT := https://github.com/merlin-network/black-fury.git
...
format:
	...
	find . -name '*.go' -type f -not -path "./vendor*" -not -path "*.git*" -not -path "./client/docs/statik/statik.go" -not -name '*.pb.go' | xargs goimports -w -local github.com/merlin-network/black-fury
.PHONY: format
...
ifeq ($(OS),Windows_NT)
	...
	IF not exist "build/node0/$(highbury_BINARY)/config/genesis.json" docker run --rm -v $(CURDIR)/build\black\Z black/node "./black testnet --v 4 -o /black --keyring-backend=test --ip-addresses blacknode0,blacknode1,blacknode2,blacknode3"
	docker-compose up -d
else
	mkdir -p localnet-setup
	@$(MAKE) localnet-build

	if ! [ -f localnet-setup/node0/$(highbury_BINARY)/config/genesis.json ]; then docker run --rm -v $(CURDIR)/localnet-setup:/black:Z black/node "./black testnet --v 4 -o /black --keyring-backend=test --ip-addresses blacknode0,blacknode1,blacknode2,blacknode3"; fi
	docker-compose up -d
endif
...
localnet-unsafe-reset:
	docker-compose down
ifeq ($(OS),Windows_NT)
	@docker run --rm -v $(CURDIR)\localnet-setup\node0\black:black\Z black/node "./black unsafe-reset-all --home=/black"
	@docker run --rm -v $(CURDIR)\localnet-setup\node1\black:black\Z black/node "./black unsafe-reset-all --home=/black"
	@docker run --rm -v $(CURDIR)\localnet-setup\node2\black:black\Z black/node "./black unsafe-reset-all --home=/black"
	@docker run --rm -v $(CURDIR)\localnet-setup\node3\black:black\Z black/node "./black unsafe-reset-all --home=/black"
else
	@docker run --rm -v $(CURDIR)/localnet-setup/node0/black:/black:Z black/node "./black unsafe-reset-all --home=/black"
	@docker run --rm -v $(CURDIR)/localnet-setup/node1/black:/black:Z black/node "./black unsafe-reset-all --home=/black"
	@docker run --rm -v $(CURDIR)/localnet-setup/node2/black:/black:Z black/node "./black unsafe-reset-all --home=/black"
	@docker run --rm -v $(CURDIR)/localnet-setup/node3/black:/black:Z black/node "./black unsafe-reset-all --home=/black"
endif
...
.PHONY: build-docker-local-black localnet-start localnet-stop
