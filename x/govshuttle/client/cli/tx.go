...
func NewLendingMarketProposalCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		Long: `Submit a proposal for the Black Lending Market along with an initial deposit.
Upon passing, the
...
func NewTreasuryProposalCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		Args:  cobra.ExactArgs(1),
		Short: "Submit a proposal to the Black Treasury",
		Long: `Submit a proposal for the Black Treasury along with an initial deposit.
Upon passing, the
The proposal details must be supplied via a JSON file.`,
