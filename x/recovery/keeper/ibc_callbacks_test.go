...
func (suite *KeeperTestSuite) TestOnRecvPacket() {
	...
	ethsecpAddrblack := sdk.AccAddress(ethPk.PubKey().Address()).String()
	ethsecpAddrCosmos := sdk.MustBech32ifyAddressBytes(sdk.Bech32MainPrefix, ethsecpAddr)

	// Setup Cosmos <=> black IBC relayer
	denom := "uatom"
	sourceChannel := "channel-292"
	blackChannel := "channel-3"
	path := fmt.Sprintf("%s/%s", transfertypes.PortID, blackChannel)
	...
	coins := sdk.NewCoins(
		sdk.NewCoin("ablack", sdk.NewInt(1000)),
	...
	testCases := []struct {
		...
		},
		{
			...
			func() {
				transfer := transfertypes.NewFungibleTokenPacketData(denom, "100", ethsecpAddrblack, ethsecpAddrCosmos)
		...
		},
		{
			...
			func() {
				transfer := transfertypes.NewFungibleTokenPacketData(denom, "100", "black", ethsecpAddrCosmos)
				bz := transfertypes.ModuleCdc.MustMarshalJSON(&transfer)
				packet = channeltypes.NewPacket(bz, 100, transfertypes.PortID, sourceChannel, transfertypes.PortID, blackChannel, timeoutHeight, 0)
			},
			false,
			false,
			coins,
		...
		},
		{
			...
			func() {
				transfer := transfertypes.NewFungibleTokenPacketData(denom, "100", ethsecpAddrblack, "badbadhf0468jjpe6m6vx38s97z2qqe8ldu0njdyf625")
				bz := transfertypes.ModuleCdc.MustMarshalJSON(&transfer)
				packet = channeltypes.NewPacket(bz, 100, transfertypes.PortID, sourceChannel, transfertypes.PortID, blackChannel, timeoutHeight, 0)
			},
			false,
		...
		},
		{
			...
			func() {
				...
				otherSecpAddrblack := sdk.AccAddress(pk1.PubKey().Address()).String()

				transfer := transfertypes.NewFungibleTokenPacketData(denom, "100", secpAddrCosmos, otherSecpAddrblack)
				bz := transfertypes.ModuleCdc.MustMarshalJSON(&transfer)
				packet = channeltypes.NewPacket(bz, 100, transfertypes.PortID, sourceChannel, transfertypes.PortID, blackChannel, timeoutHeight, 0)
			},
			true,
		...
		},
		{
			...
			func() {
				...
				transfer := transfertypes.NewFungibleTokenPacketData(denom, "100", ethsecpAddrCosmos, ethsecpAddrblack)
				bz := transfertypes.ModuleCdc.MustMarshalJSON(&transfer)
				packet = channeltypes.NewPacket(bz, 100, transfertypes.PortID, sourceChannel, transfertypes.PortID, blackChannel, timeoutHeight, 0)
			},
			true,
		...
		},
		{
			...
			func() {
				transfer := transfertypes.NewFungibleTokenPacketData(denom, "100", secpAddrCosmos, secpAddrblack)
				bz := transfertypes.ModuleCdc.MustMarshalJSON(&transfer)
				packet = channeltypes.NewPacket(bz, 100, transfertypes.PortID, sourceChannel, transfertypes.PortID, blackChannel, timeoutHeight, 0)
		...
		{
			"recovery - send uatom from cosmos to black",
			func() {
				transfer := transfertypes.NewFungibleTokenPacketData(denom, "100", secpAddrCosmos, secpAddrblack)
				bz := transfertypes.ModuleCdc.MustMarshalJSON(&transfer)
				packet = channeltypes.NewPacket(bz, 100, transfertypes.PortID, sourceChannel, transfertypes.PortID, blackChannel, timeoutHeight, 0)
		...
		{
			"recovery - send ibc/uosmo from cosmos to black",
			func() {
				denom = ibcOsmoDenom
		...
		{
			"recovery - send uosmo from osmosis to black",
			func() {
				// Setup Osmosis <=> black IBC relayer
				denom = "uosmo"
				sourceChannel = "channel-204"
				// blackChannel = claimstypes.DefaultAuthorizedChannels[0]
				blackChannel = "channel-0"
	...
	for _, tc := range testCases {
		suite.Run(fmt.Sprintf("Case %s", tc.name), func() {
			...
			suite.app.IBCKeeper.ChannelKeeper.SetChannel(suite.ctx, transfertypes.PortID, blackChannel, channel)

			// Set Next Sequence Send
			suite.app.IBCKeeper.ChannelKeeper.SetNextSequenceSend(suite.ctx, transfertypes.PortID, blackChannel, 1)
			...
			// Fund receiver account with black, ERC20 coins and IBC vouchers
			testutil.FundAccount(suite.app.BankKeeper, suite.ctx, secpAddr, coins)
...
func (suite *KeeperTestSuite) TestOnRecvPacketFailTransfer() {
	...
	denom := "uatom"
	sourceChannel := "channel-292"
	// blackChannel := claimstypes.DefaultAuthorizedChannels[1]
	...
	for _, tc := range testCases {
		suite.Run(fmt.Sprintf("Case %s", tc.name), func() {
			...
			transfer := transfertypes.NewFungibleTokenPacketData(denom, "100", secpAddrCosmos, secpAddrblack)
			packet := channeltypes.NewPacket(transfer.GetBytes(), 100, transfertypes.PortID, sourceChannel, transfertypes.PortID, blackChannel, timeoutHeight, 0)
			...
			// Fund receiver account with black
			coins := sdk.NewCoins(
				sdk.NewCoin("ablack", sdk.NewInt(1000)),
				sdk.NewCoin(ibcAtomDenom, sdk.NewInt(1000)),
			)
